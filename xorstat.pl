...
foreach my $fn(@fns)
{
  ...
  sub statdump($$%)
  {
    print "Dumping $_[0]\n";
    print OUT "<h2>$_[0]</h2>";
    print OUT "<table border='1'><tr><th>$_[0]</th><th>count</th></tr>";
    foreach(sort {$a <=> $b || $a cmp $b} keys %{$_[1]})
    {
	    #print "$_: $_[1]{$_}\n";
      print OUT "<tr><td>$_</td><td>$_[1]{$_}</td></tr>\n";
    }
    print OUT "</table>\n"; 
  }


  statdump("Page Offsets:",\%pageoffsets);
  statdump("Full Offsets:",\%fulloffsets);
  statdump("XOR Pages:",\%xorpages);
  statdump("Block Pages:",\%blockpages);
  statdump("Pages:",\%pages);
  statdump("Statistic:",\%stat);

  my $lbasperline=100;
  if(defined($stat{'maxlba'}))
  {
    print OUT "<h2>LBA Table from LBA#0 to LBA#".$stat{'maxlba'}."(".int($stat{'maxlba'}/2/1024/1024)."GB):</h2>\n";
    print OUT "<table border='1'>";

    foreach(0 .. $stat{'maxlba'}/100)
    {
      print OUT "<td bgcolor='".(defined($lbas{$_})?"#00ff00":"#ff0000")."' width='5' height='5'> </td>";
